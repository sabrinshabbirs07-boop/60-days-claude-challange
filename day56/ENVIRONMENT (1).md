...
## 1a. Production Environment Variables (Render)

Set under Render Dashboard → your service → **Environment** tab:

| Variable | Value | Notes |
|---|---|---|
| `GROQ_API_KEY` | *(real Groq key)* | **Important lesson learned Day 6:** paste carefully — a trailing newline/space in this value caused a `GROQ_NETWORK_ERROR` in production that didn't appear locally. If Groq calls fail in production but work locally, re-check this value for hidden whitespace first. |

Render automatically injects its own `PORT` value — do **not** manually set a `PORT` environment variable, since the code already reads `process.env.PORT` with a local fallback.

**Free-tier note:** Render's free instance spins down after inactivity, causing a 50+ second delay on the first request after idle time. This is expected behavior, not a bug.
